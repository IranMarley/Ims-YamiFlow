...
public static class PermissionEndpoints
{
    public static void Map(IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/", async (string roleId, GetRolePermissionsHandler handler, CancellationToken ct) =>
        {
            var result = await handler.Handle(new GetRolePermissionsQuery(roleId), ct);
        ...
        group.MapPost("/", async (
            string roleId, [FromBody] PermissionBody body, AddPermissionHandler handler, CancellationToken ct) =>
        {
            var result = await handler.Handle(new AddPermissionCommand(roleId, body.Resource, body.Operation), ct);
        ...
        group.MapDelete("/", async (
            string roleId, [FromBody] PermissionBody body, RemovePermissionHandler handler, CancellationToken ct) =>
        {
            var result = await handler.Handle(new RemovePermissionCommand(roleId, body.Resource, body.Operation), ct);
